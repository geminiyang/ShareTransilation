...
import com.yqq.myutillibrary.R;
...
public class SwipeBackLayout extends FrameLayout {
     ...
     * predefined non-interactive（非交互式） motion（手势）.
     */
    public static final int STATE_SETTLING = ViewDragHelper.STATE_SETTLING;

    /**
     * Default threshold（阈值） of scroll
     */
    ...
    public SwipeBackLayout(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs);
        //为每个SwipeBackLayout设置一个ViewDragHelper
...
//            e.printStackTrace();
    ...
    private void drawScrim(Canvas canvas, View child) {
        //这里的实现都是参考DrawerLayout.java的源码
        // 参与运算的两个数均以补码出现。
        // & 只有对应的两个二进位均为1时，结果位才为1 ，否则为0。
        // | 只要对应的二个二进位有一个为1时，结果位就为1。
        // ^ 当两对应的二进位相异时，结果为1
        final int baseAlpha = (mScrimColor & 0xff000000) >>> 24;//按位想与再右移24位
    ...
    /**
     * 让特定SwipeBackLayout与特定Activity关联
     * @param activity
     */
    public void attachToActivity(Activity activity) {
        ...
        //移除掉r.id.content的默认嵌套,将activity中的视图和SwipeBackLayout合并后添加到decor
    ...
    public void removeFromActivity(Activity activity){
        ...
        ViewGroup decorChild = (ViewGroup) getChildAt(0);//获取默认嵌套
        ViewGroup decor = (ViewGroup) activity.getWindow().getDecorView();
